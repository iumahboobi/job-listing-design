...
import('./data.json').then((khar) => {
    renderJobCards(khar);
})

var Inputfilter = document.querySelector('#myInput');  // Selecting
...
function renderJobCards(jobs) {
    var mainBox = document.querySelector('.main-box');  // Selecting the mainbox div
    jobs.forEach(job => {

        var parentDiv, logoWidth, img, jobDescription, jobDescriptionDetail, header, undefineList, languageData,
            parentDiv = document.createElement('div');
        ...
        if (job.featured) {
            jobDescriptionDetail.append(spanList('FEATURED', ['featured', 'padding']))
        }
        ...
        job.languages.forEach(language => {
            languageData.append(spanList(language, ['padding', 'back-ground', 'margin-right']));
        });
...
function jobFilter(value) {
    ...
    allJobs.forEach(job => {
        ...
        if (job.dataset.role.toLowerCase().indexOf(value) > -1 || job.dataset.level.toLowerCase().indexOf(value) > -1 || job.dataset.languages.toLowerCase().indexOf(value) > -1 || job.dataset.tools.toLowerCase().indexOf(value) > -1) {
